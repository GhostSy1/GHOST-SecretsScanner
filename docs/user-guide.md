# GHOST-SecretsScanner User Guide

## Purpose

This guide explains how to run `GHOST-SecretsScanner` from a local terminal and how to interpret its output. The tool reads the input supplied by the operator and does not treat an empty result as proof that a system is secure.
